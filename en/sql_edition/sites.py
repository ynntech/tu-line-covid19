...
class Eng(Site):
    ...
    def get(self):
        ...
        contents = ["《4/19》",
                    ...
                    "行動指針レベル４引き上げに伴い、変更しています",
                    "新入生：https://www.eng.tohoku.ac.jp/news/detail-,-id,1582.html",
                    "在学生：https://www.eng.tohoku.ac.jp/news/detail-,-id,1581.html"]
        stick1.time = stick1.timeobj(timestr="4.19")
        stick1.content = "\n".join(contents)
        ...
        stick2.time = stick2.timeobj(timestr="4.10")
        stick2.content = "\n".join(contents)
...
class Lifesci(Site):
    ...
    def get(self):
        ...
        top = soup.find("h3")
        month, day = top.text.split("(")[-1].split()[:-1]
        stick = LifesciNews()
        stick.time = stick.timeobj(month, day)
        stick.content = f"《{stick.date}》\n新コロナウイルス感染症（COVID-19）への対応についてが更新されました\n{self.url}"
        info_list = [stick]
        return self.dic(info_list)
